...
function(jobName, agentEnv={}, stepEnvFile='', patchFunc=identity) patchFunc({
  ...
  local env = {
    ...
    BUILDKITE_PLUGIN_K8S_SECRET_NAME: 'buildkite-agent',
    ...
    BUILDKITE_PLUGIN_K8S_AGENT_TOKEN_SECRET_KEY: 'token',
    BUILDKITE_PLUGIN_K8S_INIT_IMAGE: 'embarkstudios/k8s-buildkite-agent:matt-testing',
    ...
    template: {
      ...
      spec: {
        ...
        containers: [
          {
            ...
            volumeMounts: [
              ...
              { mountPath: '/root/.ssh/id_rsa', name: 'ssh-keys', subPath: 'id_rsa'},
              { mountPath: '/root/.ssh/id_rsa.pub', name: 'ssh-keys', subPath: 'id_rsa.pub'},
              { mountPath: '/root/.ssh/browserstack-tests-deploy-key', name: 'browserstack-tests-ssh-keys', subPath: 'browserstack-tests-deploy-key'},
              { mountPath: '/root/.ssh/browserstack-tests-deploy-key.pub', name: 'browserstack-tests-ssh-keys', subPath: 'browserstack-tests-deploy-key.pub'},
              { mountPath: '/usr/bin/docker', name: 'docker-binary'},
              { mountPath: '/var/run/docker.sock', name: 'docker-socket'},
              { mountPath: '/tmp/yarn-cache', name: 'yarn-cache'},
        ...
        volumes: [
          ...
          { name: 'browserstack-tests-ssh-keys', secret: { secretName: 'browserstack-tests-ssh', defaultMode: '0400' } },
          { name: 'ssh-keys', secret: { secretName: 'buildkite-agent-ssh', defaultMode: '0400' } },
          { name: 'yarn-cache', hostPath: {path: '/tmp/yarn-cache'} },
          { name: 'docker-binary', hostPath: {path: '/usr/bin/docker'} },
